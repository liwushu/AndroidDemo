...
package com.tcl.launcher.effect.utils;
...
public class MathUtils {
    ...
    static private class Cos {
        ...
        static {
            ...
            for (int i = 0; i < 360; i += 90)
                table[(int) (i * degToIndex) & SIN_MASK] = (float) Math.cos(i
                        * degreesToRadians);
        }
    }
    ...
    static public void getMinMaxOf4(float a1, float a2, float a3, float a4,
            float[] min, int minOffset, float[] max, int maxOffset) {
        float min12;
        float max12;
        float min34;
        float max34;
        ...
        min[minOffset] = min12 <= min34 ? min12 : min34;
        max[maxOffset] = max12 >= max34 ? max12 : max34;
    }
}
